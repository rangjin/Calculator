...
public class Octal extends Calculator {
    public Octal(String str, String remain) {
        super(str, remain);
    }
    ...
    public int checkError() {
        ...
        while (true) {
            if (expr[i] == ' ') {
                ...
                break;
            } else if ((expr[i] >= 65 && expr[i] <= 70) || (expr[i] >= 97 && expr[i] <= 102) || (expr[i] >= 56 && expr[i] <= 57)) {
                return 1;
            }
            i++;
        }
    ...
    public void putTokens(char[] expr, int s, int e) {
        ...
        if (ch == 'o') {
            ...
            int i = Integer.parseInt(String.valueOf(arr), 8);
            el = new Operand(i);
        } else if (ch == 'e') {
            if (expr[s] == '+') {
                el = new Plus();
    ...
    public void evaluation() {
        ...
        value = Integer.toOctalString((int) num);
        setValue(value);
    }
}
